...
export default function App() {
  const [theme, setTheme] = useState(() => localStorage.getItem('forno-theme') || 'light')
  const [query, setQuery] = useState('')
  const [category, setCategory] = useState('all')

  useEffect(() => {
    document.documentElement.setAttribute('data-theme', theme)
    localStorage.setItem('forno-theme', theme)
  }, [theme])
    ...
    <div className="app-wrapper">
      <section className="hero">
        ...
        <div className="hero-content">
          <Logo size={88} />
          <p className="eyebrow">Wood-Fired · Napoli Style</p>
          <h1>فورنو</h1>
          <p className="tagline">پیتزای تنور چوبی — خمیر ۲۴ ساعته، آتش واقعی</p>
        </div>
      </section>
      ...
      <div className="app">
        ...
        <footer className="footer">
          <strong>فورنو</strong>
          <span>تنور چوبی · از ساعت ۱۲ تا ۲۳</span>
        </footer>
      </div>
    </div>
  )
